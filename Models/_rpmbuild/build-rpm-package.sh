...
# This script builds one RPM package for the Gazebo models.
#
# Author: Stefan Deser


# First fetch the names from the respective .spec files
MODELS_PKG_NAME=$(grep Name hbp-gazebo-models.spec | awk '{print $2}')


BUILD_DIR=build-model-pkg
RPM_TARGET_DIR=target

# The following directories are the ones which are used in the spec file for
# the models package.
# Here we copy the models over to TMP_DIR_MODEL (for use in Gazebo) as well as
# TMP_DIR_WEB and in the spec file those models should be copied to their final
# destination in the build process.
TMP_DIR_MODEL=tmp/models
TMP_DIR_WEB=tmp/assets
...
function copy_models() {
    ...
    rm -rf ./$TMP_DIR_MODEL/*
    mkdir -p ./$TMP_DIR_MODEL
    log "Copy models and remove blender folders"
    MODELS=$(cat "./models.txt")

    for model in ${MODELS[@]}; do
        log "Copying $model"
        cp -r ../$model/ ./$TMP_DIR_MODEL
        # Remove the large blender folders if existent
        rm -rf ./$TMP_DIR_MODEL/$(basename $model)/blender
    done
}

function create_webified_models() {
    mkdir -p ./$TMP_DIR_WEB
    cp -r ./$TMP_DIR_MODEL/* ./$TMP_DIR_WEB
    python ./webify_models_v2.py ./$TMP_DIR_WEB
}
...
copy_models && create_webified_models && build_rpm_package $MODELS_PKG_NAME
#
...
check_if_package_exists $MODELS_PKG_NAME \
